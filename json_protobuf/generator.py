...
{{in}}    ::{{pbobj}}::{{desc.name}}::{{field_descriptor.name}} JSON_PROTOBUF_EXPORT {{field_descriptor.name}}_to_pb(const json_spirit::Object &node);
{{in}}    void JSON_PROTOBUF_EXPORT {{field_descriptor.name}}_to_json(json_spirit::Object &node, const ::{{pbobj}}::{{desc.name}}::{{field_descriptor.name}} &value);
{% endfor %}
